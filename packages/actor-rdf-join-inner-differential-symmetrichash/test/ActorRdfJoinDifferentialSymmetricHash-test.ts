...
import { ActorRdfJoinDifferentialSymmetricHash } from '../lib/ActorRdfJoinDifferentialSymmetricHash';
import '@comunica/jest';
import {DataFactory} from "rdf-data-factory";
import {BindingsFactory} from "@comunica/bindings-factory";
import {Bindings, IActionContext, IQueryOperationResultBindings} from "@comunica/types";
...
describe('ActorRdfJoinDifferentialSymmetricHash', () => {
  let bus: any;
  ...
  describe('The ActorRdfJoinDifferentialSymmetricHash module', () => {
    it('should be a function', () => {
      expect(ActorRdfJoinDifferentialSymmetricHash).toBeInstanceOf(Function);
    });

    it('should be a ActorRdfJoinDifferentialSymmetricHash constructor', () => {
      expect(new (<any> ActorRdfJoinDifferentialSymmetricHash)({ name: 'actor', bus })).toBeInstanceOf(ActorRdfJoinDifferentialSymmetricHash);
      expect(new (<any> ActorRdfJoinDifferentialSymmetricHash)({ name: 'actor', bus })).toBeInstanceOf(ActorRdfJoin);
    });

    it('should not be able to create new ActorRdfJoinDifferentialSymmetricHash objects without \'new\'', () => {
      expect(() => { (<any> ActorRdfJoinDifferentialSymmetricHash)(); }).toThrow();
    });
  });

  describe('An ActorRdfJoinInnerDifferentialSymmetrichash instance', () => {
    let mediatorJoinSelectivity: Mediator<
      Actor<IActionRdfJoinSelectivity, IActorTest, IActorRdfJoinSelectivityOutput>,
      IActionRdfJoinSelectivity, IActorTest, IActorRdfJoinSelectivityOutput>;
    let actor: ActorRdfJoinDifferentialSymmetricHash;
    ...
    beforeEach(() => {
      ...
      actor = new ActorRdfJoinDifferentialSymmetricHash({ name: 'actor', bus, mediatorJoinSelectivity });
      variables0 = [];
      variables1 = [];
      action = {
        type: 'inner',
